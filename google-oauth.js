  ...
  async function init({ buttonContainerId, onSuccess, onError, usePrompt = false } = {}) {
    ...
    try {
      ...
      if (buttonContainerId) {
        ...
        if (container) {
          // Botón propio y siempre visible (no depende del iframe de Google).
          // Al hacer clic se abre el selector de cuentas de Google (prompt) y,
          // si el origen está autorizado, se entrega la credencial a la app.
          ...
          btn.addEventListener('click', () => {
            if (window.google && window.google.accounts && window.google.accounts.id) {
              ...
              onLoginError("Google Identity Services no está disponible. Revisa tu conexión.");
            }
          });
          container.appendChild(btn);
          console.info("[GoogleAuth] Botón propio añadido. Origen actual:", origin);
